...
def build_parser():
    ...
    parser.add_argument("--assumption-state", default="valid", help="Initial assumption state.")
...
def main(argv=None):
    ...
    subprocess.run(["openspec", "new", "change", args.name], check=True)
    ensure_meta_file(
        CHANGES_DIR / args.name,
        defaults={
            "status": args.status,
